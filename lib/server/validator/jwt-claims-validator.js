 ...
 */
var TIME_CLAIM_LEEWAY = 30; // seconds
...
var issuerSubjectValidation = function(issuer, subject) {
    if (subject && subject !== issuer) {
        return q.reject(new Error('If defined, subject must be the same as issuer'));
    }
    return q.resolve();
};
...
var issuerAndSubjectValidation = function(issuer, subject, authorizedSubjects, keyId) {
    var validateIssuerAndSubject = _.partial(issuerSubjectValidation, issuer, subject);
...
var formalTimeClaimsValidation = function(issuedAt, expiry, mayBeNotBefore) {
    var validateExpiry = _.partial(expiryValidation, expiry, issuedAt);
    var validateLifeTime = _.partial(lifeTimeValidation, issuedAt, expiry);
...
module.exports = {
    validate: function(authorizedSubjects, resourceServerAudience, header, claims) {
