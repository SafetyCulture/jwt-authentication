...
var validateKeyId = function(verifiableToken) {
    var header = verifiableToken.header;
...
var Validator = function (config) {
    ...
    this.validate = function (jwtToken, authorizedSubjects, callback) {
        var verifiableToken;
        try {
            verifiableToken = jsonWebToken.decode(jwtToken, {complete: true});
        } catch (error) {
            return callback(error);
        }

        var verifyToken = _.partial(jsonWebToken.verify, jwtToken);
        var validateClaims = _.partial(claimsValidator.validate, authorizedSubjects,
            config.resourceServerAudience, verifiableToken.header);
        var onSuccess = _.partial(returnClaims, callback);
        var onError = _.partial(returnError, callback);

        validateKeyId(verifiableToken)
